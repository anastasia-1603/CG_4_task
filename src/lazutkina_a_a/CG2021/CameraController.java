    ...
    public void mouseDragged(MouseEvent e) {
        ...
        if (last != null) {
            ...
            if (leftFlag) {
                ...
                camera.modifyRotate(
                        Objects.requireNonNull(Matrix4Factories.rotationXYZ(da, Matrix4Factories.Axis.Y))
                                .multiply(
            ...
            if (rightFlag) {
                Vector4 zero = new Vector4(sc.screenToReal(new ScreenPoint(0, 0)), 0);
                Vector4 cur = new Vector4(sc.screenToReal(new ScreenPoint(dx, dy)), 0);

                /*Вектор смещения в реальных координатах с точки зрения камеры*/
                Vector3 delta = cur.add(zero.multiply(-1)).asVector3();
            ...
            if (middleFlag && dy != 0) {
                ...
                float length = cur.add(zero.multiply(-1)).asVector3().length();
                if (dy < 0)
                    length = -length;
